...
```{r eval = F}
  ...
  # between texas and florida via similar methods to the ones we just used
    ...
    us_aea_final <-
      rbind(us_aea_remove, alaska, hawaii)
...
alaska_2 <- 
  map_polygons %>% 
  filter(STATEFP == 2) %>% 
  mutate(group = paste0(group, "-", piece))
...
write.csv(map_area_clipped_hulls, "map_area_clipped_hulls.csv", 
          row.names = F)
```
...
map_districts_df <-
  fortify(clipped_hulls, region="id") %>%
  mutate(CD115FP = id) %>% 
  left_join(compare_list %>% select(-id))
