...
raw_links <- 
  ...
  as.data.frame() %>% 
  mutate(tweet_date = mdy(tweet_date)) %>% 
  filter(tweet_date >= ymd(min_date)) %>% 
  filter(query == "with Andy")
...
write.csv(final_descriptions, "Twitter_descriptions3.csv", row.names = F)
write.csv(final_images, "Twitter_images3.csv", row.names = F)
