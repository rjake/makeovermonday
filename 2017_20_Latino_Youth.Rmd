...
#get twitter links to scrape
  gdrive_data <- 
    gs_read(google_key, 
            ...
            lookup = T) %>% 
    mutate(Pct_Unemployed = Unemployed/Economically.Active.Population) 

#Plot freq

  fivenum(gdrive_data$Pct_Unemployed)
  
  ggplot(gdrive_data) + 
    geom_boxplot(aes(x = 1, y = Pct_Unemployed, group = 1)) +
    coord_flip()


#logical statement to test, left side is column name, right side is test
  test.dv <- "(Pct_Unemployed > 0.17)"
